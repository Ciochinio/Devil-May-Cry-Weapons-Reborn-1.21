		...
		if (DevilMayCryWeaponsRebornModItems.KALINA_ANN_I.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y + 1, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("devil_may_cry_weapons_reborn:kalinaann.shot1")), SoundSource.PLAYERS, 1, 1);
				} else {
					_level.playLocalSound(x, (y + 1), z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("devil_may_cry_weapons_reborn:kalinaann.shot1")), SoundSource.PLAYERS, 1, 1, false);
				}
			}

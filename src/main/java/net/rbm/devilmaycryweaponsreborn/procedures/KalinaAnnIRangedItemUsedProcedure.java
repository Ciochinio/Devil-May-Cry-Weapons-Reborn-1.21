		...
		if (DevilMayCryWeaponsRebornModItems.KALINA_ANN_I.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
			{
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = initArrowProjectile(new KalinaAnnIProjectileEntity(DevilMayCryWeaponsRebornModEntities.KALINA_ANN_I_PROJECTILE.get(), 0, 0, 0, projectileLevel, createArrowWeaponItemStack(projectileLevel, 1, (byte) 0)),
							entity, (float) 0.5, true, true, false, AbstractArrow.Pickup.DISALLOWED);
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y + 1, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("devil_may_cry_weapons_reborn:kalinaann.shot1")), SoundSource.PLAYERS, 1, 1);
				} else {
					_level.playLocalSound(x, (y + 1), z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("devil_may_cry_weapons_reborn:kalinaann.shot1")), SoundSource.PLAYERS, 1, 1, false);
				}
			}

		...
		if (DevilMayCryWeaponsRebornModItems.KALINA_ANN_II.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
							AbstractArrow entityToSpawn = new KalinaAnnIIProjectileEntity(DevilMayCryWeaponsRebornModEntities.KALINA_ANN_II_PROJECTILE.get(), level);
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y + 1, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("devil_may_cry_weapons_reborn:kalinaann.shot2")), SoundSource.PLAYERS, 1, 1);
				} else {
					_level.playLocalSound(x, (y + 1), z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("devil_may_cry_weapons_reborn:kalinaann.shot2")), SoundSource.PLAYERS, 1, 1, false);
				}
			}
